...
<!--MAIN-->
	...
	//Pitam da li je setovan ID
	if(isset($_GET['id']))
	{
		//Ako je setovan pitam da li je broj
		if(is_numeric($_GET['id']))
		{
			$id=$_GET['id'];
			$sql="SELECT * FROM vesti WHERE id=$id";
			$result=mysqli_query($db, $sql);
			//ako upit ne vrati ništa nije dobro
			if(mysqli_num_rows($result)==0)
			{
				echo "Ne postoji vest sa id-jem $id!!!!!";
			}
			else//Ako upit vrati jedan zapis
			{
				while($call=mysqli_fetch_object($result))
				{
					echo "<h2 class='media-header-news'>$call->naslov</h2>$call->kategorija<br><br>";
					if($call->slika!="")
					echo "<img src='images/$call->slika' class='media-pictures-news' alt='Interkons'>";
					echo "<p class='media-text-news'>".$call->sadrzaj."</p>";
	...
	{
		echo "Niste izabrali vest za prikaz!!!<br>";
	}
	//mysqli_close($db);
	?>
	<!--Forma za postavljanje komentara-->
	<form action="news.php?id=<?= $id;?>" method="post">
	<input type="text" id="autor" name="autor" placeholder="Author" style="font-size: 1rem;"/><br><br>
	<textarea id="tekst" name="tekst" placeholder ="Comment" style="font-size: 1.3rem;"></textarea><br><br>
	...
	comments::prikaziSveKomentare($db, $id);
	?>
	</div>
<!--SIDEBAR-->
	<div id="sidebar">
